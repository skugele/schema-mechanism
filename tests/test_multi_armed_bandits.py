...
class TestMultiArmedBandits(unittest.TestCase):
    ...
    def test_init(self):
        ...
        state, is_terminal = self.env.reset()

        self.assertEqual(sym_state('S'), state)
    ...
    def test_step_1(self):
        for m_id in [m.id for m in self.machines]:
            ...
            state, _ = self.env.step(Action(f'sit(M{m_id})'))
            self.assertEqual(sym_state(f'M{m_id}'), state)

            # test: M[m_id] + Action(stand) => S
            state, _ = self.env.step(Action('stand'))
            self.assertEqual(sym_state('S'), state)
    ...
    def test_step_2(self):
        for m_id in [m.id for m in self.machines]:
            ...
            state, _ = env.step(Action('deposit'))
            self.assertEqual(sym_state(f'M{m_id},P'), state)

    def test_step_3(self):
        env = BanditEnvironment(machines=self.machines, init_state=sym_state('M0,P'))

        # test: M0,P + Action('play') => M0,W
        state, _ = env.step(Action('play'))
        self.assertEqual(sym_state('M0,W'), state)

        # test: M0,W + Action('stand') => S
        state, _ = env.step(Action('stand'))
        self.assertEqual(sym_state('S'), state)
    ...
    def test_step_4(self):
        ...
        state, _ = env.step(Action('play'))
        self.assertEqual(sym_state('M1,L'), state)
    ...
    def test_step_6(self):
        ...
        for action in [Action(a_str) for a_str in ['play', 'sit(M0)']]:
            state, _ = env.step(action)
            self.assertEqual(sym_state('M0'), state)
